...
const DashboardPage: React.FC = () => {
  ...
  console.log("the user from user auth dash", user);

  if (!user) {
    return (
      <div>
        <h2 className="text-white">Something went wrong user is not here</h2>
      </div>
    );
  }
  return (
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5 capitalize">
              {user.role || "admin or user"} · {user.email || "temp@gmail.com"}
            </p>
          </div>
        </div>
        <span
